function astar(nodes, start, target) {
  ...
  while (!!unvisitedNodes.length) {
    ...
    updateNeighbors(nodes, currentNode,target,start);
  }
}
...
function updateNeighbors(nodes, node, target, start) {
  let neighbors = getNeighbors(node.id, nodes);
  for (let neighbor of neighbors) {
    if (target) {
      updateNode(node, nodes[neighbor], nodes[target],nodes, nodes[start]);
    } else {
...
function updateNode(currentNode, targetNode, actualTargetNode, nodes, actualStartNode) {
...
function getNeighbors(id, nodes) {
